<template>
  <div>
    <div
      v-if="!isEdit"
      style="display: flex;"
      :style="{flexDirection:boxValue.directionMode}"
    >
      <div
        v-if="boxValue.MultiSelectMode=='false'"
        v-for="(item,index) in boxValue.listData"
      ...
      >
        ...
        <label>
          <input
            ...
            type="checkbox"
            @change="getSingleSelect(item,index)"
            checked
          ...
          <input
            ...
            v-model="isSelected[index]"
            v-bind:value="item.ItemValue"
          /> <span :style="{fontSize:boxValue.fontSize+'pt'}">{{item.ItemName}}</span>
        </label>
      </div>
      <div v-else>
        <div>
        ...
        <label>
          <input
            type="checkbox"
            @change="getMultSelectValue"
            checked
            v-model="multSelctValue"
            v-bind:value="item.ItemValue"
          />{{item.ItemName}}
        </label>
      </div>
    ...
    >
      <div
        v-for="item in boxValue.listData"
      ...
      >
        <label>
          <input
            type="checkbox"
            disabled="disabled"
            v-on:value="item.ItemValue"
          /><span :style="{fontSize:boxValue.fontSize+'pt'}">{{item.ItemName}}</span></label>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getItemValue() {
      ...
      this.api.getFormSqlResult(params).then(res => {
        let field =
          this.boxValue.SourceTableName + this.boxValue.SourceFieldName;
        ...
        if (this.boxValue.MultiSelectMode == "false") {
          ...
          if (res[field] != "null" && res[field] != "" && res[field] != null) {
