<template>
  <div style="position: relative;margin:2px;">
    <div v-if="!page">
      <div
        v-if="changeEventView"
        style="width:100%;height:100%;position:fixed;z-index:14;top:0;left:0;"
      >
        <div
          class="addData"
          style="height:400px;width: 600px;background-color: #E3EFFF;"
        >
          <div class="load_top">
          ...
          <div style="padding: 15px;display:flex;flex-wrap:wrap;height:230px;">
            ...
            <div style="display: flex;height:21px;width:255px;">
              ...
              <input
                type="text"
                style="width:170px;"
                v-model="eventObj.ITEM_NAME"
              >
            </div>
            <div style="display: flex;height:21px;width:255px;">
              <div style="width: 80px;">途径</div>
              <select
                v-model="eventObj.ADMINISTRATOR"
                style="display:inline-block;height:100%;width:174px;height:22px;"
              >
                <option value=""></option>
                <option
            ...
            <div style="display: flex;height:21px;width:255px;">
              ...
              <input
                type="text"
                style="width:170px;"
                v-model="eventObj.CONCENTRATION"
              >
            </div>
            <div style="display: flex;height:21px;width:255px;">
              <div style="width: 80px;">单位</div>
              <select
                v-model="eventObj.CONCENTRATION_UNIT"
                style="display:inline-block;height:100%;width:174px;height:22px;"
              >
                <option value=""></option>
                <option
            ...
            <div style="display: flex;height:21px;width:255px;">
              ...
              <input
                style="width:170px;"
                type="text"
                v-model="eventObj.PERFORM_SPEED"
              >
            </div>
            <div style="display: flex;height:21px;width:255px;">
              <div style="width: 80px;">单位</div>
              <select
                v-model="eventObj.SPEED_UNIT"
                style="display:inline-block;height:100%;width:174px;height:22px;"
              >
                <option value=""></option>
                <option
            ...
            <div style="display: flex;height:21px;width:255px;">
              ...
              <input
                style="width: 170px;"
                type="text"
                v-model="eventObj.DOSAGE"
              >
            </div>
            <div style="display: flex;height:21px;width:255px;">
              <div style="width: 80px;">单位</div>
              <select
                v-model="eventObj.DOSAGE_UNITS"
                style="display:inline-block;height:100%;width:174px;height:22px;"
              >
                <option value=""></option>
                <option
            ...
            <div style="display: flex;height:21px;width:255px;">
              ...
              <dateTime
                ...
                v-model="eventObj.START_TIME"
              ></dateTime>
            </div>
            <div style="display: flex;height:21px;width:255px;">
              <div style="width: 80px;">是否持续</div>
              <select
                style="width: 174px;height:22px;display:block;font-size:12px;"
                v-model="eventObj.DURATIVE_INDICATOR"
              >
                <option
                  style="background-color: white;"
                  v-bind:value="0"
      ...
      <div style="max-height: 20px;">
        <div
          v-for="(item,index) in xTimeArray"
          v-if="index%6==0"
          style="width: 30px;margin-left: -8px;font-size: 12px;display: inline-block;"
          :title="item"
        >{{item}}</div>
        <div
          v-else
      ...
      <div>
        <div
          ...
          v-if="index < rows-1"
          :style="{top:svgHeight/rows*index+20+'px',height:svgHeight/rows-2.7+'px',lineHeight:svgHeight/rows+'px',}"
          style="line-height: 14px;width: 160px;border-bottom: 1px solid #8391a2;  font-size: 14px;position: absolute;left: -165px;padding-left: 5px;white-space:nowrap;word-break: keep-all;"
        >
          ...
          <span
            ...
            v-if="item.CONCENTRATION"
          >({{item.CONCENTRATION}}{{item.CONCENTRATION_UNIT}},</span>
          <span
            style="font-size:12px;"
            v-if="item.SPEED_UNIT"
          >{{item.SPEED_UNIT}},</span>
          <span
            style="font-size:12px;"
            v-if="item.ADMINISTRATOR"
          >{{item.ADMINISTRATOR}})</span>
          <span v-if="item.ITEM_NAME"></span> -->

          <!-- {{item.CONCENTRATION_UNIT}},{{item.SPEED_UNIT}}{{item.ADMINISTRATOR}} -->
          <span v-if="item.ITEM_NAME"> {{item.ITEM_NAME}}({{item.CONCENTRATION}}{{item.CONCENTRATION_UNIT}} {{item.SPEED_UNIT}} {{item.ADMINISTRATOR}})</span>
        </div>
        <div
          v-for="(item,index2) in dataArray"
          v-if="index2==rows-1"
          :style="{top:svgHeight/rows*index2+20+'px',height:svgHeight/rows-2.7+'px',lineHeight:svgHeight/rows+'px',}"
          style="width: 160px; font-size: 14px;position: absolute;left: -165px;padding-left: 5px;white-space:nowrap;word-break: keep-all;"
        >
      ...
      <div style="position: relative;">
        ...
        <div
          @mousemove.stop="mouseMoveInfo(item,$event)"
          @mouseenter="showTipInfo(item,$event)"
          @mouseleave="hideTipInfo()"
          v-if="item.obj.DURATIVE_INDICATOR=='0'||!item.obj.DURATIVE_INDICATOR"
          style="csursor: pointer;position: absolute;font-size: 8pt;color: blue;"
          :style="{top:item.top+'px',left:item.x1-1+'px',height:svgHeight/rows-3+'px',lineHeight:svgHeight/rows+'px'}"
          v-for="(item,index) in xArray"
        >
          <span
            v-if="item.obj.ITEM_NAME=='七氟烷1'"
            style="padding: 0 2px 0 0px;"
          >{{item.obj.CONCENTRATION}}{{item.obj.CONCENTRATION_UNIT}}</span>
          <span
            v-else
            style="padding: 0 2px 0 0px;"
          >{{item.obj.DOSAGE}}</span>
        </div>
        <div
          v-if="item.obj.DURATIVE_INDICATOR=='1'"
          style="position: absolute;font-size: 8pt;color: blue;"
          :style="{top:item.top+2+'px',left:item.x1+item.w/2-15+'px',height:svgHeight/rows-3+'px',lineHeight:svgHeight/rows+'px'}"
          v-for="(item,index) in xArray"
        >
          <span
            v-if="item.obj.ITEM_NAME=='七氟烷1'"
            style="padding: 0 2px 0 0px;display: block;width: 16px;text-align: center;"
          >{{item.obj.CONCENTRATION}}{{item.obj.CONCENTRATION_UNIT}}</span>
          <span
            v-else
            style="padding: 0 2px 0 0px;"
        ...
        <div v-if="tipView">
          <div
            style="position: absolute;max-width:300px;min-width:220px;width:auto;background-color: white;border: 0.5px solid;padding: 3px;font-size: 12px;z-index: 15"
        ...
        <div
          ...
          :style="{top:item.y1-svgHeight/rows/8+'px',left:item.x1+'px',width:item.w+'px',height:svgHeight/rows/4+'px'}"
          @mouseenter="showTipInfo(item,$event)"
          @mouseleave="hideTipInfo()"
          v-for="item in xArray"
          @mousemove.stop="mouseMoveInfo(item,$event)"
          @dblclick="openChangeEvent(item)"
        >
        </div>
      </div>
    </div>
    <div v-else>
      <div style="max-height: 20px;">
        <div
          v-for="(item,index) in xTimeArray"
          v-if="index%3==0"
          style="width: 28px;margin-left: -10px;font-size: 12px;display: inline-block;"
        >{{item}}</div>
        <div
          v-else
          style="width: 12px;display: inline-block;"
      ...
      <div>
        <div
          v-for="(item,index) in dataArray"
          :style="{top:svgHeight/rows*index+20+'px'}"
          style="height: 14px;line-height: 12px;width: 165px;border-bottom: 1px solid #8391a2;  font-size: 12px;position: absolute;left: -165px;"
        >
        </div>
      </div>
      <div style="position: relative;">
...
export default {
  ...
  methods: {
    ...
    timeControl(startTime) {
      ...
      if (pageOper == 0 && startMinTime) {
        for (var i = 0; i <= this.columns; i++) {
          timeArray.push(
            new Date(
              new Date(this.config.initTime).getTime() + 1000 * 60 * m * i
            ).Format("hh:mm")
          );
        }
        ...
        for (var i = 0; i <= this.columns; i++) {
          timeArray.push(
            new Date(
              new Date(defaultTime).getTime() + 1000 * 60 * m * i
            ).Format("hh:mm")
          );
        }
      } else {
        for (var i = 0; i <= this.columns; i++) {
          timeArray.push(
            new Date(this.config.initTime.getTime() + 1000 * 60 * m * i).Format(
              "hh:mm"
            )
          );
        }
    ...
    drawLineArrow(x1, y1, x2, y2) {
      ...
      path +=
        " L" +
        (Math.round(x2) + Math.round(Par * cosy - (Par / 2.0) * siny)) +
        "," +
        (Math.round(y2) + Math.round(Par * siny + (Par / 2.0) * cosy));

      path +=
        " M" +
        (Math.round(x2) +
          Math.round(Par * cosy + (Par / 2.0) * siny) +
          "," +
          (Math.round(y2) - Math.round((Par / 2.0) * cosy - Par * siny)));
      path += " L" + x2 + "," + y2;
        ...
        if (this.config.pageOper == -1) {
          ...
          for (var i = 0; i < list.length; i++) {
            if (this.config.pagePercentNum != 1 && list[i].PATIENT_ID) {
              list[i].vStartTime = this.config.initTime.Format(
                "yyyy-MM-dd hh:mm:ss"
              );
            }
          }
          ...
          for (var i = 0; i < list.length; i++) {
            if (list[i].PATIENT_ID) {
              if (list[i].ENDDATE == null || list[i].ENDDATE == "") {
                if (
                  new Date(this.config.patientMaxTime) >
                  new Date(this.config.initTime)
                ) {
                  list[i].vStartTime = new Date(this.config.initTime).Format(
                    "yyyy-MM-dd hh:mm:ss"
                  );
                ...
                if (
                  new Date(list[i].ENDDATE) > new Date(this.config.initTime)
                ) {
                  list[i].vStartTime = new Date(this.config.initTime).Format(
                    "yyyy-MM-dd hh:mm:ss"
                  );
        ...
        if (list[i].START_TIME) {
          if (i == -1) break;
            ...
            if (list[i].DURATIVE_INDICATOR == 1) {
              ...
              //判断是否在当前时间内
              if (
                new Date(list[i].START_TIME) > new Date(this.config.maxTime)
              ) {
                continue;
              }
              if (list[i].DURATIVE_INDICATOR == 0) {
                if (
                  new Date(list[i].START_TIME) < new Date(this.config.initTime)
                ) {
                  continue;
                }
              }
              ...
              if (list[i].ENDDATE == null || list[i].ENDDATE == "") {
                if (
                  new Date(this.config.patientMaxTime) > this.config.maxTime
                ) {
                  eMin = this.getMinuteDif(
                    this.config.initTime,
                    this.config.maxTime
                  );
                } else {
              ...
              let topi;
              y1 = Math.round(
                (h / this.rows / 2) * (m + 1) + ((h / this.rows) * m) / 2
              );
              y2 = y1;
              //判断是否同一种药品
              if (this.dataArray.length > 0) {
                for (var j = 0; j < this.dataArray.length; j++) {
                  if (
                    list[i].ITEM_NAME == this.dataArray[j].ITEM_NAME &&
                    list[i].ITEM_CLASS == this.dataArray[j].ITEM_CLASS
                  ) {
                    y1 = Math.round(
                      (h / this.rows / 2) * (j + 1) + ((h / this.rows) * j) / 2
                    );
                    y2 = y1;
                    flag = false;
                    topi = j;
    ...
    openChangeEvent(item) {
      ...
      this.api.selectMapByKey(params).then(res => {
        this.eventObj = res;
        this.tempObj1 = res;
      });
    ...
    saveEvent() {
      ...
      if (
        this.tempObj.START_TIME != null &&
        new Date(this.tempObj.START_TIME) < new Date(this.eventObj.START_TIME)
      ) {
        //插入一条术中登记记录
        let arr1 = [];
        let arr2 = [];
        ...
        this.api.insertMedAnesthesiaEventBatch(arr1).then(res => {
          let updateparms = {
          ...
          this.api.updateMedAnesthesiaEventBatch(arr2).then(rest => {
            if (rest.success) {
              this.xTimeInit();
              alert("保存成功");
            } else {
      ...
      //否则更新
        ...
        arr.push(params);
        this.api.updateMedAnesthesiaEventBatch(arr).then(res => {
          if (res.success) {
            this.xTimeInit();
            alert("保存成功");
          } else {
