<template>
  <div style="font-family: 宋体;font-weight: 200">
    <div
      ...
      :style="{width:value.width+'px',height:value.height+'px',fontSize:value.fontSize+'pt'}"
    ...
    <div v-if="value.type=='line'">
      <div
        style="margin-top:1px;height:1pt;border-top:1pt solid #000;"
        :style="{width:value.width+'px'}"
      ></div>
    </div>
    <div v-if="value.type=='verticalLine'">
      <div
        style="margin-left:1px;width:1pt;border-right:1pt solid #000;"
    ...
    <div v-if="value.type=='textarea'">
      <textareaTable
        :conInfo="value"
        v-on:toparentevent="getChlidValue"
      ></textareaTable>
    </div>
    <div v-if="value.type=='autoInput'">
      <textarea
        :style="{height:value.height*0.75+'pt',width:value.width*0.75+'pt'}"
        v-model="value.text"
      ></textarea>
    </div>
    ...
    <div v-if="value.type=='checkBoxAll'">
      <div v-if="isPage.isPage">
        <div style="min-width: 100px;min-height: 40px;z-index: 5;display: flex;border:1px dashed;">
          <check-box
            :boxValue="value"
            :isEdit="isPage.isPage"
            v-on:toparentevent="getChlidValue"
          ></check-box>
        </div>
      </div>
      <div v-else>
        <div style="min-width: 100px;min-height: 40px;z-index: 5;display: flex;">
          <check-box
            :boxValue="value"
            :isEdit="isPage.isPage"
            v-on:toparentevent="getChlidValue"
    ...
    <div v-if="value.type=='div'">
      <div
        ...
        :style="{width:value.width*0.75+'pt',height:value.height*0.75+'pt',border:value.borderStyle}"
      ></div>
    </div>
    ...
    <div v-if="value.type=='specialSymbol'">
      <specialSymbol
        :page="isPage.isPage"
        :object="value"
        :style="{width:value.width*0.75+'pt',height:value.height*0.75+'pt'}"
      ></specialSymbol>
    </div>
    <div v-if="value.type=='bodyPosition'">
      <bodyPosition
        :page="isPage.isPage"
        :object="value"
        :style="{width:value.width*0.75+'pt',height:value.height*0.75+'pt',fontSize:value.fontSize+'pt'}"
      ></bodyPosition>
    </div>
