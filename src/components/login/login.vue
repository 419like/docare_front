<template>
  <div
    :style="note"
    style="background-color: rgb(0,0,0);position: relative;"
  >
    <div class="loginBox">
        ...
        <div style="width:60%;">
          ...
          <div style="margin-top:20px;height:24px;">
            ...
            <input
              type=""
              name=""
              style="width: 180px;height:22px;box-sizing:border-box;"
              v-model="loginName"
...
export default {
  ...
  methods: {
    // 获取用户信息列表
    getList() {
      this.api.medUsersList().then(res => {
    ...
    doSql() {
      ...
      this.api.doSql(params).then(res => {
        console.log(res);
      });
    },
    moveFun(e) {
    ...
    login() {
      ...
      this.api.login(params).then(res => {
        if (res.success) {
          ...
          let userInfo = JSON.stringify(res.obj);
          //用户信息存储到sessionStorage中
          sessionStorage.setItem("userInfoSession", userInfo);
          this.$router.push({
            path: "menu"
          });
        } else {
