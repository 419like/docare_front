<template>
  <div class="dictionaries">
    <div style="background-color: #e3fdec;width: 1214px;z-index: 11;cursor: auto;position: absolute;top: calc(50% - 305px);height: 610px;left: calc(50% - 607px);border:2px solid #36c85f;z-index: 1;">
      <div
        class="load_top"
        style="height: 30px;background-color: rgb(54,157,200);line-height: 30px;"
      >
        <div>{{titleName}}</div>
        <div
          @click="aboutNone"
      ...
      <div style="height: 349px;display:flex;border-bottom:3px solid #74da98;">
        <div style="width:80%;">
          <div style="display: flex;">
            <div
              style="border:1px solid #b1f3ba;background-color:#fff;box-sizing:border-box;font-size:12px;"
              :style="{minWidth:cell.width+'px'}"
              v-for="cell in tbconfig"
            >
              {{cell.title}}
            </div>
          </div>
          ...
          <div
            @mousedown="downFun()"
            @mouseup="upFun()"
            style="width: 100%;border:1px solid #222;overflow-y: auto;height:275px;box-sizing: border-box;background-color:white;padding-bottom:5px;"
            ref="eventContent"
          >
            <div style="min-height: 268px;">
              <div
                ...
                style="display:flex;"
                :class="{chooseItem:item.thooseItem}"
                @mouseleave="outFun(item)"
                @click="clickItem(item,$event)"
              >
                <!-- 判断是否为事件 -->
                <div
                  v-for="cl in tbconfig"
                  v-if="item.ITEM_CLASS!='1'"
                >
                  <div
                    style="height:20px;"
                    v-if="cl.timeEdit"
                  >
                    ...
                    <dateTime
                      :class="{selectchooseItem:item.thooseItem}"
                      @change="getChangeValue(item)"
                      style="border:0;display:block;font-size:12px;"
                      :width="cl.width-2"
                      v-model="item[cl.fieldObj]"
                    ></dateTime>
                  </div>
                  <div
                    v-else-if="cl.isChixu"
                    class="hideselect"
                  >
                    <select
                      :class="{selectchooseItem:item.thooseItem}"
                      style="height:22px;width:65px;display:block;font-size:12px;"
                      v-model="item[cl.fieldObj]"
                      :style="{width:(cl.width)+'px'}"
                      v-on:change="getChangeValue(item)"
                    >
                      <option
                  ...
                  <div
                    class="hideselect"
                    v-else
                    style="border:1px solid #a9a9a9;box-sizing:border-box;height:100%;"
                    :style="{width:(cl.width)+'px'}"
                  >
                    <select
                      :class="{selectchooseItem:item.thooseItem}"
                      v-if="cl.fieldObj == 'ADMINISTRATOR'"
                      v-model="item[cl.fieldObj]"
                      style="border:0;display:inline-block;height:100%;width:100%;height:22px;"
                      @change="getChangeValue(item)"
                    >
                      <option value=""></option>
                      <option
                    ...
                    <select
                      :class="{selectchooseItem:item.thooseItem}"
                      v-else-if="cl.fieldObj == 'CONCENTRATION_UNIT'"
                      v-model="item[cl.fieldObj]"
                      style="border:0;display:inline-block;height:100%;width:100%;height:22px;"
                      @change="getChangeValue(item)"
                    >
                      ...
                      <option
                        style="background-color: white;"
                        v-for="(item,index) in concentrationList"
                        :value="item.itemName"
                      >{{ item.itemName }}</option>
                    </select>
                    <select
                      :class="{selectchooseItem:item.thooseItem}"
                      v-else-if="cl.fieldObj == 'SPEED_UNIT'"
                      v-model="item[cl.fieldObj]"
                      style="border:0;display:inline-block;height:100%;width:100%;height:22px;"
                      @change="getChangeValue(item)"
                    >
                      ...
                      <option
                        style="background-color: white;"
                        v-for="(item,index) in speedUnitList"
                        :value="item.itemName"
                      >{{ item.itemName }}</option>
                    </select>
                    <select
                      :class="{selectchooseItem:item.thooseItem}"
                      v-else-if="cl.fieldObj == 'DOSAGE_UNITS'"
                      v-model="item[cl.fieldObj]"
                      style="border:0;display:inline-block;height:100%;width:100%;height:22px;"
                      @change="getChangeValue(item)"
                    >
                      ...
                      <option
                        style="background-color: white;"
                        v-for="(item,index) in dosageUnitsList"
                        :value="item.itemName"
                      >{{ item.itemName }}</option>
                    </select>
                    <input
                      v-else
                      style="height:20px;border:0;display:block;font-size:12px;"
                      @change="getChangeValue(item)"
                      type="text"
                      :style="{width:(cl.width-2)+'px'}"
                      v-model="item[cl.fieldObj]"
                    >
                  </div>
                </div>
                ...
                >
                  ...
                  <div v-else-if="cl.isChixu">
                    <select
                      style="height:22px;width:65px;border:0;display:block;font-size:12px;"
                      disabled="true"
                      v-model="item[cl.fieldObj]"
                      v-on:change="getChangeValue(item)"
                      :style="{width:(cl.width)+'px'}"
          ...
          <div style="height: 60px;padding-top: 10px;padding-left:15px;display:flex;justify-content: space-between;box-sizing:border-box;">
            <div style="font-size:12px;">
              <button
                style="width: 70px;font-size:12px;height:25px;"
                @click="saveTempletViewFun"
              >保存模板</button>
              <button
                style="width: 70px;font-size:12px;height:25px;"
                @click="openTempLet"
              ...
              >
                ...
                <option
                  v-for="option in eventTypeList"
                  v-bind:value="option.typeId"
                >
                  {{ option.typeName }}
                </option>
              </select>
            ...
            <div>
              <button
                style="margin-right: 20px;width: 70px;font-size:12px;height:25px;"
                @click='saveBtn'
              >保存</button>
              <button
                style="margin-right: 20px;width: 70px;font-size:12px;height:25px;"
                @click="deleteMedAnesthesiaEvent"
              >删除</button>
              <button
                style="margin-right: 20px;width: 70px;font-size:12px;height:25px;"
                @click="canselEventList"
              >取消</button>
            </div>
          </div>
        </div>
        <div style="padding: 0px 5px;">
          <div>
            <div style="height: 120px;flex-wrap:wrap;display: flex;overflow:auto;">
              <button
                ...
                style="width: 60px;font-size:12px;height:25px;line-height: 25px;text-align: center;margin:5px 5px 0 5px;display:block;"
                @click="medAnesthesiaEventOpenByItemClass(item)"
              >
                {{item.typeName}}
              </button>
            </div>
            <div style="background-color:#fff;font-size:12px;">
              <div
                ...
                :class="{width1:widthChange}"
                v-if="selectTypeTemp.typeId=='2' || selectTypeTemp.typeId=='C'"
              >
              ...
              <div
                style="height: 140px;overflow-y: auto;box-sizing:border-box;"
                v-if="selectTypeTemp.typeId=='2' || selectTypeTemp.typeId=='C'"
              >
                <div
                  v-for="item in eventNameList"
                  style="width: 100%;display: flex"
                  @dblclick="addEvent(item)"
                >
                  <div style="width: 170px;border:1px solid black;">
                    {{item.itemName}}
                  </div>
          ...
          <div style="border: 1px solid #686869;width:100%;height:22px;box-sizing:border-box;">
            <input
              style="width:100%;height:100%;border:0;display:block;line-height:20px;outline:none;"
              v-model="serchZm"
              placeholder="无字符过滤"
              @keyup="serchEvent"
            >
      ...
      <div
        style="height: 145px;background-color:#fff;font-size:12px;position: relative;width: 1214px;"
        ref="signContent"
        @keydown.17="moreFun"
        @keyup.17="exitMore"
      >
        ...
        <div
          ref="timeScroll"
          style="width: calc(100% - 110px);position: absolute;top:0;left: 100px;overflow:hidden;"
        >
          <div
            class="flex"
            :style="{width: signdataList.length*60+'px'}"
          >
            <div
              v-for="sItem in signdataList"
              @click="getSignClickData(sItem)"
              style="position:relative;"
            >
              <input
                type="text"
                style="width:100%;height:100%;position:absolute;border:0;readonly:true;opacity:0;"
              >
              <div
                style="width: 60px;height: 18px;border:1px solid #C1C1C1;box-sizing:border-box;"
                :class="{chooseClass:sItem.thoose}"
                :title="sItem.time"
              >
                {{sItem.time | discount}}
              </div>
        ...
        <div
          ref="headTitleScroll"
          style="position: absolute;top: 18px;left: 10px;;width: 90px;overflow: hidden;"
          :style="{height:(signdataList.length*60>1114?117:127)+'px'}"
        >
          <div :style="{height:itemNameList.length*22+'px'}">
            <div
              style="height: 22px;width: 90px;font-weight: 600;line-height:22px;"
              :class="{clickClass:item.choose}"
        ...
        <div
          ref="dataScroll"
          style="position: absolute;top: 18px;left: 100px;width: calc(100% - 100px);height: 127px;overflow: auto"
        >
          <div class="flex">
            <div
              v-for="sItem in signdataList"
              @click="getSignClickData(sItem)"
              :class="{blueChooseClass:sItem.thoose}"
            >
              <div
                v-for="(secItem,index) in sItem.dataValue"
                style="height: 22px;border:1px solid #c1c1c1;width:60px;box-sizing:border-box;"
              >
                <input
                  :value="secItem"
                  style="width: 100%;background-color:rgba(0,0,0,0);display:block;height:100%;border:0;"
                  @change="signChange($event,index,sItem)"
                >
              </div>
            </div>
          </div>
      ...
      <div
        ...
        id="codeSelect"
        ref="codeSelect"
        style="position: absolute;bottom:35px;left: 150px;"
      >
        <div>
          <input
            ref="inSelect"
        ...
        <div style="padding-left: 5px;z-index: 3;background-color: white;height: 200px;overflow-y: auto;">
          <div
            class="listIngt"
            @click="getSeclectItem(option)"
            v-for="option in showItemList"
          >{{option.itemName}}</div>
        </div>
      </div>
      <div style="margin-top: 5px;padding-left:15px;">
        <button
          @click="saveSignChange"
          style="width: 60px;height:25px;font-size:12px;"
        >保存</button>
        <button
          style="width: 60px;height:25px;font-size:12px;"
          @click="deleteMedPatientMonitorData"
        >删除</button>
        <button
          style="width: 60px;height:25px;font-size:12px;"
          @click="insertView"
      ...
      <div
        ...
        style="width:100%;height:100%;position:fixed;z-index:14;top:0;left:0;"
      >
        <div
          class="addData"
      ...
      <!-- 保存模板输入内容 -->
      <div
        v-if="saveTempletView"
        style="width: 500px;min-height: 200px;background-color: white;z-index: 3;position: absolute;top: 20%;left: 20%;border:2px solid rgb(61,164,206);"
      >
        ...
        <div style="padding: 15px;display: flex;">
          ...
          <inputDiv
            :conInfo="methodObj"
            methodName="medAnaesthesiaDictList"
            attrName="anaesthesiaName"
            toAttrName="anaesthesiaName"
          ></inputDiv>
        </div>
      ...
      <!-- 显示事件模板 -->
      <div
        style="position: absolute;top: 10%;left: 7%;width: 100%;"
        v-if="tempLetView.view=='t'"
      >
        <event-templet
          :tempView="tempLetView"
          :itemList="eventList"
        ></event-templet>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    medAnesthesiaEventOpenByItemClass(item) {
      ...
      this.api.medAnesthesiaEventOpenByItemClass(params).then(res => {
    ...
    upFun() {
      ...
      if (this.selectedItem.length > 1) {
        for (
          var a = this.selectedItem.length - 1;
          a < this.selectedItem.length;
          a++
        ) {
    ...
    deleteMedAnesthesiaEvent() {
      if (this.selectedItem.length > 1) {
        ...
        if (this.SaveData == true) {
          this.api.deleteBatchMedAnesthesiaEvent(addData).then(res => {
            if (res.success) {
              alert("删除成功");
            } else {
              alert("删除失败");
        ...
        if (this.selectedItem.length !== 0) {
          ...
          if (this.selectedItem[0].addFlag) {
            ...
            this.api.deleteMedAnesthesiaEvent(params).then(res => {
              if (res.success) {
                alert("删除成功");
              } else {
                alert("删除失败");
    ...
    saveBtn() {
      if (this.changeEvent.length > 0) {
        ...
        if (arrs.length > 0) {
          this.api.updateMedAnesthesiaEventBatch(arrs).then(res => {
            this.selectMedAnesthesiaEventList();
            this.changeEvent = [];
            if (res.success) {
              alert("保存成功");
      ...
      if (arry1.length > 0) {
        this.api.insertMedAnesthesiaEventBatch(arry1).then(res => {
          if (res.success) {
            alert("保存成功");
          } else {
            alert("保存失败");
    ...
    getSignTimeData(len) {
      ...
      return false;
      this.api.getSignTimeData(params).then(res => {
        var sortArray = [];
    ...
    deleteMedPatientMonitorData() {
      if (this.getClickSignData == "") {
        ...
        this.api.deleteBatchMedPatientMonitorData(paramsArray).then(res => {
          if (res.success) {
            this.getSignName();
            alert("删除成功");
          } else {
        ...
        this.api.deleteMedPatientMonitorData(params).then(res => {
          if (res.success) {
            this.getSignName();
            alert("删除成功");
          } else {
    ...
    insertView() {
      ...
      if (this.signdataList.length > 0) {
        ...
        this.insertStartTime = this.changeDateFormat(
          new Date().Format("yyyy-MM-dd hh:mm")
        );
        let endTimeMinute = new Date().getTime() + 5 * 60 * 1000;
        this.insertEndTime = this.changeDateFormat(
          new Date(endTimeMinute).Format("yyyy-MM-dd hh:mm")
        );
      }
    },
    //获取插入体征的数据
    ...
    addItem() {
      ...
      //计算开始时间与结束时间差值单位是毫秒
      var k = parseInt(
        this.datetimeLocalToDate(this.insertEndTime) -
          this.datetimeLocalToDate(this.insertStartTime)
      );
      //单位是分钟
      ...
      if (k3 > 0) {
        for (var i = 1; i <= k3; i++) {
          var time2 = new Date(
            time1.getTime() + parseInt(this.spaceTime / 60) * i * 1000 * 60
          );
          for (var j = 0; j < m1; j++) {
            this.addItemList.push({
              patientId: this.objectItem.patientId,
      ...
      this.api.insertBatchMedPatientMonitorData(this.addItemList).then(res => {
        if (res.success) alert("插入成功");
        else alert("插入失败");
        this.addItemList = [];
        this.addView = !this.addView;
        this.getSignName();
    ...
    saveSignChange() {
      ...
      this.api.updateMedPatientMonitorDatas(params).then(res => {
        if (res.success) {
          alert("保存成功");
        } else {
          alert("保存失败");
    ...
    addSignItem() {
      ...
      if (this.allSignItems.length == 0) {
        let params = {};
        this.api.selectAllItems(params).then(res => {
    ...
    deleteSignItem() {
      ...
      if (
        this.deleteTzItem.itemCode == 40 ||
        this.deleteTzItem.itemCode == 44 ||
        this.deleteTzItem.itemCode == 188 ||
        this.deleteTzItem.itemCode == 89 ||
        this.deleteTzItem.itemCode == 90 ||
        this.deleteTzItem.itemCode == 324 ||
        this.deleteTzItem.itemCode == 209 ||
        this.deleteTzItem.itemCode == 211 ||
        this.deleteTzItem.itemCode == 212 ||
        this.deleteTzItem.itemCode == 202 ||
        this.deleteTzItem.itemCode == 210 ||
      ...
      this.api.deleteMedPatientMonitorDataCode(params).then(res => {
        if (res.success) {
          alert("删除成功");
        } else alert("删除失败");
        this.deleteTzItem = "";
    ...
    serchEvent() {
      ...
      for (var i = 0; i < list.length; i++) {
        if (
          list[i].nameJm &&
          list[i].nameJm.indexOf(this.serchZm.toUpperCase()) >= 0
        ) {
          newList.push(list[i]);
        }
      }
    ...
    saveTemplet() {
      ...
      this.api.insertBtchMedAnesthesiaEventTemplet(params).then(res => {
        if (res.success) {
          alert("保存成功");
        } else {
          alert("保存失败");
    ...
    getRoadList() {
      this.api
        .getMedAnesthesiaCommDictByItemClass({
          itemClass: "用药途径"
        })
        .then(res => {
